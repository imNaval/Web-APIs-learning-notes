let noteFiles = [
  ...
  async function populateSidebar() {
    ...
    noteFiles.forEach(note => {
      ...
      a.href = `notes.html?topic=${topicId}`;
      ...
      if (note.subtopics.length > 0) {
        ...
        note.subtopics.forEach(sub => {
          ...
          subA.href = `notes.html?topic=${topicId}#${sub.replace(/\s+/g, '-').toLowerCase()}`;
  ...
  function toggleSubtopics(topicId) {
    ...
    if (currentUl && !isCurrentlyVisible) {
      ...
      window.location.href = `notes.html?topic=${topicId}`;
    }
  }
...
//     // Fetch all files and extract subtopics
